  ...
  SearchFilters copyWith({
    ...
    return SearchFilters(
      ...
      preferenciasDieteticas:
          preferenciasDieteticas ?? this.preferenciasDieteticas,
    );
  }
}
...
class AlimentoRepository {
  ...
  Future<List<ScoredFood>> search(
    ...
    return TelemetryService().measureAsync(
      ...
      () async {
        ...
        var localResults = await _db.searchFoodsFTS(
          normalizedQuery,
          limit: limit,
          offset: offset,
        );
        if (kDebugMode) {
          debugPrint(
            '[AlimentoRepository] Local FTS results: ${localResults.length}',
          );
        }
        ...
        final scoredResults = localResults
            .map(
              (f) => ScoredFood(
                food: f,
        ...
        return _applyRanking(
          scoredResults,
          normalizedQuery,
        ).take(limit).toList();
      },
      tags: {'query_length': query.length, 'offset': offset},
    );
  }
  ...
  Future<List<Food>> searchOffline(
    String query, {
    int limit = 50,
    int offset = 0,
  }) async {
    return _db.searchFoodsOffline(query, limit: limit, offset: offset);
  }
  ...
  Future<Food?> searchByBarcode(String barcode) async {
    ...
    final local = await (_db.select(
      _db.foods,
    )..where((f) => f.barcode.equals(cleanBarcode))).getSingleOrNull();
    ...
    try {
      if (kDebugMode) {
        debugPrint(
          '[AlimentoRepository] Barcode not local, searching OFF: $cleanBarcode',
        );
      }
  ...
  Future<void> saveFood(FoodModel model) async {
    final companion = FoodsCompanion(
      ...
      fiberPer100g: Value(model.fiberPer100g),
      sugarPer100g: Value(model.sugarPer100g),
      saturatedFatPer100g: Value(model.saturatedFatPer100g),
      sodiumPer100g: Value(model.sodiumPer100g),
    ...
    await _db.insertFoodFts(model.id, model.name, model.brand);
  }
  ...
  List<Food> _applyFilters(List<Food> foods, SearchFilters filters) {
    return foods.where((f) {
      ...
      if (filters.minProteinas != null &&
          (f.proteinPer100g ?? 0) < filters.minProteinas!) {
        return false;
      }
  ...
  static const _kScoreGenericBoost =
      40.0; // Generic/natural products (sin marca)
  static const _kScoreProcessedPenalty =
      -30.0; // Processed products (zumos, salsas, etc.)
  ...
  Future<List<ScoredFood>> _searchOpenFoodFacts(
    String query, {
    int limit = 20,
    CancelToken? cancelToken,
  }) async {
  ...
  Future<Food?> _parseAndSaveProduct(Map<String, dynamic> product) async {
    ...
    if (code.isNotEmpty) {
      existing = await (_db.select(
        _db.foods,
      )..where((f) => f.barcode.equals(code))).getSingleOrNull();
    }
    ...
    if (existing == null) {
      ...
      final candidates = await (_db.select(
        _db.foods,
      )..where((f) => f.normalizedName.equals(normName))).get();
    ...
    final companion = FoodsCompanion(
      ...
      fiberPer100g: Value(_parseDouble(nutriments['fiber_100g'])),
      sugarPer100g: Value(_parseDouble(nutriments['sugars_100g'])),
      saturatedFatPer100g: Value(
        _parseDouble(nutriments['saturated-fat_100g']),
      ),
      sodiumPer100g: Value(_parseDouble(nutriments['sodium_100g'])),
